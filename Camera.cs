...
namespace Camera2D
{
    public class Camera
    {
        ...
        public Rectangle ScreenDimentions { get; private set; }
        ...
        public Camera(GraphicsDevice graphicsDevice, GraphicsDeviceManager graphics, RenderTarget2D renderTarget)
        {
            ...
            scale = 1f;
            SetScale(scale);
            ScreenDimentions = new Rectangle(0, 0, graphics.PreferredBackBufferWidth, graphics.PreferredBackBufferHeight);
        }
        ...
        {
            camera = pos;
            Translation = Matrix.CreateTranslation(-camera.X + (Game1.SCREEN_WIDTH /2), -camera.Y + (Game1.SCREEN_HEIGHT / 2), 0);
        }
    }
}
